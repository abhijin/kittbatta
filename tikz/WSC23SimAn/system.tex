...
\newcommand{\user}{\includegraphics[width=1cm]{user.png}}
...
\node[blk, right of=study, text width=3cm, xshift=3.5cm] (cgsub)
{Cascade graph\\generation \& \\augmentation};
\node[blk, right of=cgsub, text width=2.5cm, xshift=1cm] (measub) 
{Cascade \\graph\\properties};
\node[blk, right of=measub, text width=2.5cm, xshift=1cm] (consolidate) 
{Consolidate \\properties};
\node[blk, below right of=cgsub, text width=4cm, yshift=-1.9cm, xshift=1cm,
minimum height=2cm] (pnsub) {Population network \\ attributes and
properties};
\node[left of=pnsub,xshift=-6cm,yshift=.4cm] (pn) 
{\includegraphics[width=3cm]{popnet.pdf}};
\node[desc,right of=pn,xshift=-.5cm, yshift=.2cm] {Population network(s)};
\node[extblk, text width=2.5cm, right of=consolidate, xshift=1cm] 
(vizint) {Visualization \\\& \\ Interpretation};

\draw[ioedge,->] (pn.north-|study) -- (study);
\draw[ioedge,->] ($(study.east) + (3,0)$) -- (cgsub);
\draw[ioedge,->] ($(pnsub.east-|pn)+(1,0)$) -- (pnsub);
\draw[blkedge,->] (cgsub) -- (measub);
\draw[blkedge,->] (measub) -- (consolidate);
\draw[blkedge,->] (consolidate) -- (vizint);
\draw[blkedge,->] (pnsub) -| (vizint);
\draw[blkedge,->] ($(pnsub.west) + (0,.7)$) -| (cgsub);
\node[desc, above of=pnsub, xshift=-1.5cm, yshift=-1cm] {Node and edge
attributes and measures};
\node[desc, right of=pnsub, xshift=-1cm, yshift=.5cm, text width=5cm,
anchor=west] {Contact network
properties};
\draw[blkedge,->] (vizint.north) -- +(0,1) -| (study);
\end{tikzpicture}
\end{document}
