...
\node[block] at (0,0) {ABMS on multi-layer networks};
% sqrt(3)*radius
\node[block] at (240:2.6) {Learning local and system properties};
...
\node[explain,anchor=south,above=of data,shift={(0,1.6)},text width=4cm] (edata) {Observations,
surveillance, experimentation, domain knowledge, theory, surrogate events, etc.};
\draw[dedge,-*] (data) -- (edata);
%%
\node[explain,anchor=south,text width=5cm] (eAI) at
(AI|-edata.south) {A simulation driven learning framework for model
    selection.};
\draw[dedge,-*] (AI) -- (eAI);
%%
...
Counterfactual analysis and multi-network interventions.};
\draw[dedge,-*] (sim) -- (esim);
%%
