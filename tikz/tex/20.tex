\documentclass[tikz,border=2]{standalone}
\usetikzlibrary{shadows,arrows,shapes,positioning,calc,backgrounds,fit}
% Define the layers to draw the diagram
%
\newcommand{\vanish}[1]{}
%\newcommand{\vanish}[1]{#1}
\begin{document}
\begin{tikzpicture}
[node distance=1cm,
vertex/.style={shape=circle,draw=black,inner sep=2pt},
traced/.style={vertex,fill=blue!40},
myedge/.style={thick},
dedge/.style={>=latex', shorten >=.0pt, shorten <=.0pt, thick}]

\node (a1) [vertex] at (0,0) {\vanish{1}};
\node (a2) [vertex] at (-.5,-1){\vanish{2}};
\node (a3) [traced,label=below:$u$] at (-.8,-2){\vanish{3}};
\node (a5) [traced,label=above left:$b$] at (.3,-1.3){\vanish{5}};
\node (a6) [vertex] at (1,-2.2){\vanish{6}};
\node (a7) [vertex] at (1,-1.3){\vanish{7}};
\node (a9) [traced,label=right:$v$] at (0.5,.5){\vanish{9}};
\node (a10) [traced,label=above right:$c$] at (0.7,-.5){\vanish{10}};
\node (a12) [vertex] at (1.2,-.7){\vanish{12}};
\node (a14) [traced,label=above left:$a$] at (0.2,-2){\vanish{14}};

%\node (a11) [vertex] at (1.2,-0.3){\vanish{11}};
%
%\node (a15) [vertex] at (-.2,-2.7){\vanish{15}};
%
%\node (a4) [vertex] at (.5,-3){\vanish{4}};
%\node (a16) [vertex] at (0.3,-3.4){\vanish{16}};
%\node (a17) [vertex] at (0.7,-3.4){\vanish{17}};
%
%\node (a8) [vertex] at (1.5,-2){\vanish{8}};
%
%\node (a13) [vertex] at (2,-1.8){\vanish{13}};
%\node (a18) [vertex] at (1.8,-2.5){\vanish{18}};
%\node (a19) [vertex] at (2,-2.2){\vanish{19}};


%\node (a15) [vertex] at (2.2,-1){\vanish{15}};


\draw[myedge] (a1) -- (a9);
\draw[dedge,<-] (a9) -- (a10);
\draw[myedge] (a1) -- (a10);
\draw[myedge] (a1) -- (a2);
\draw[myedge] (a2) -- (a3);
\draw[myedge] (a2) -- (a5);
\draw[dedge,->] (a3) -- (a14);
\draw[dedge,->] (a5) -- (a10);
\draw[dedge,<-] (a5) -- (a14);
\draw[myedge] (a10) -- (a7);
\draw[myedge] (a10) -- (a12);
\draw[myedge] (a6) -- (a14);
\draw[myedge] (a6) -- (a7);
%
%\draw[myedge] (a2) -- (a3);
%\draw[myedge] (a2) -- (a5);
%\draw[myedge] (a2) -- (a14);
%
%%\draw[myedge] (a3) -- (a4);
%%\draw[myedge] (a3) -- (a6);
%\draw[myedge] (a3) -- (a15);
%
%\draw[myedge] (a4) -- (a6);
%\draw[myedge] (a4) -- (a15);
%\draw[myedge] (a4) -- (a14);
%\draw[myedge] (a4) -- (a16);
%\draw[myedge] (a4) -- (a17);
%
%\draw[myedge] (a5) -- (a6);
%\draw[myedge] (a5) -- (a7);
%%\draw[myedge] (a5) -- (a8);
%%\draw[myedge] (a5) -- (a10);
%%\draw[myedge] (a5) -- (a14);
%
%\draw[myedge] (a6) -- (a7);
%\draw[myedge] (a6) -- (a8);
%\draw[myedge] (a6) -- (a14);
%
%\draw[myedge] (a7) -- (a8);
%\draw[myedge] (a7) -- (a10);
%\draw[myedge] (a7) -- (a14);
%
%\draw[myedge] (a8) -- (a13);
%\draw[myedge] (a8) -- (a18);
%\draw[myedge] (a8) -- (a19);
%
%\draw[myedge] (a10) -- (a11);
%\draw[myedge] (a10) -- (a12);

%\node (a3) [vertex,below right=of v11,label=right:$3$] {\vanish{}};
%\node (a4) [vertex,below=of v21,label=left:$4$] {};
%\node (a5) [vertex,below=of v31,label=right:$5$] {};
%\node (a6) [vertex,below=of v41,label=left:$6$] {};
%\node (a7) [vertex,below=of v51,label=right:$7$] {};
%\draw[myedge] (v11) -- (v21) -- (v31) -- (v51) -- (v41) -- (v61) -- (v71)
%-- (v51);
%\draw[myedge] (v11) -- (v31);
%\draw[myedge] (v21) -- (v41);
%
%\node (v12) [vertex,label=above:$1$,fill] at (4,0) {};
%\node (v22) [vertex,below left=of v12,label=left:$2$] {};
%\node (v32) [vertex,below right=of v12,label=right:$3$] {};
%\node (v42) [vertex,below=of v22,label=left:$4$] {};
%\node (v52) [vertex,below=of v32,label=right:$5$] {};
%\node (v62) [vertex,below=of v42,label=left:$6$,fill] {};
%\node (v72) [vertex,below=of v52,label=right:$7$] {};
%\draw[dedge,<-] (v12) -- (v22);
%\draw[dedge,<-] (v32) -- (v12);
%\draw[dedge,<-] (v22) -- (v32);
%\draw[dedge,<-] (v42) -- (v22);
%\draw[dedge,<-] (v52) -- (v32);
%
%\draw[dedge,<-] (v62) [out=45,in=135] to (v72);
%\draw[dedge,->] (v62) [out=-45,in=-135] to (v72);

\end{tikzpicture}
{}
\end{document}
